...
@section('content_header')
    <h1 class="m-0 text-danger">LISTA DE PADRES</h1>
@stop
...
<div class="card">
    ...
    <div class="card-header">
        <div class="d-flex">
            ...
            @can('admin.parents.create')
                <div class="col justify-content-end">
                    <a href="{{ route('admin.parent.create') }}" class="btn btn-success"><i class="fas fa-user"></i> NUEVO PADRE</a>
                </div>
            @endcan
        </div>
    </div>
        ...
        <table class="table table-hover text-nowrap table-hover table-success table-striped-columns border-danger">
            <thead>
                <tr class="bg-success text-white">
                    ...
                    <th scope="col"><i class="fas fa-briefcase"></i> {{ __('TRABAJO') }}</th>
                    <th scope="col"><i class="fas fa-building"></i> {{ __('CARGO') }}</th>
            ...
            <tbody class="table-group-divider">
                @foreach ($parent as $items)
                    <tr>
                        ...
                        <td>
                            <div class="row">
                                @can('admin.parents.destroy')
                                    {!! Form::open(['route' => ['admin.parent.destroy', $items->id], 'method' => 'DELETE']) !!}
                                    &nbsp;&nbsp;<button class="btn btn-sm btn-danger">
                                        <i class="nav-icon fas fa-trash-alt"></i>
                                    </button>&nbsp;
                                @endcan
                                @can('admin.parents.edit')
                                    {!! Form::close() !!}
